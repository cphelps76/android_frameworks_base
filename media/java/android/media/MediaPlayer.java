    ...
    public void setDisplay(SurfaceHolder sh) {
        ...
        if(SystemProperties.getBoolean("media.p2pplay.enable", false))
        {
            ...
            if(mContext != null) 
            {
                ...
                String mClassName = Settings.Secure.getString(resolver, "class_name");
                Log.d(TAG, "get classname is " + mClassName);
                if(mClassName != null && !("".equals(mClassName))){
                    try {
                        int value = Settings.Secure.getInt(resolver, mClassName);
                    }
                    catch(SettingNotFoundException e) {
                        Intent i = new Intent("com.amlogic.store.classname");
