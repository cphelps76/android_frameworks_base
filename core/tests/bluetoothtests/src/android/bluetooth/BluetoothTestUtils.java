...
public class BluetoothTestUtils extends Assert {
    ...
    /** Time between radio state change in ms **/
    private static final int POLL_RADIO_TIME = 400;
    ...
    private class BluetoothReceiver extends FlagReceiver {
        ...
        private static final int STATE_RADIO_ON_FLAG =1 << 9;
        private static final int STATE_RADIO_OFF_FLAG = 1 << 10;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            else if(BluetoothAdapter.ACTION_RADIO_STATE_CHANGED.equals(intent.getAction())){
                    BluetoothAdapter adapter = BluetoothAdapter.getDefaultAdapter();
                    int prevState= intent.getIntExtra(BluetoothAdapter.EXTRA_PREVIOUS_STATE, -1);
                    int newState= intent.getIntExtra(BluetoothAdapter.EXTRA_STATE, -1);
                    writeOutput(String.format("previous State := %d  and nextState := %d ",prevState,newState));
                    if(adapter.isRadioEnabled()){
                        setFiredFlag(STATE_RADIO_ON_FLAG);
                    }
                    else{
                        setFiredFlag(STATE_RADIO_OFF_FLAG);
                    }
                }
            }
        }
    ...
    /**
     * Enables radio and make sure that the radio is on.
     *
     * @param adapter The BT adapter.
     */
    public void enable_radio(BluetoothAdapter adapter) {

        int mask = (BluetoothReceiver.STATE_RADIO_ON_FLAG);
        long start = -1;
        BluetoothReceiver receiver = getBluetoothReceiver(mask);
       if(!adapter.isRadioEnabled())
        {
            assertFalse(adapter.isRadioEnabled());
             start= System.currentTimeMillis();
            assertTrue(adapter.enableRadio());
        }
        else if(adapter.isRadioEnabled())
        {
            assertTrue(adapter.isRadioEnabled());
            removeReceiver(receiver);
        }
        long s =System.currentTimeMillis();
        while(System.currentTimeMillis() - s < ENABLE_DISABLE_TIMEOUT) {
            if(adapter.isRadioEnabled()&& (receiver.getFiredFlags() & mask) == mask) {
                assertTrue(adapter.isRadioEnabled());
                long finish=System.currentTimeMillis();
                if(start !=-1 && finish!=-1){
                 writeOutput(String.format("enable_radio() completed in %d ms", (finish - start)));
                } else {
                    writeOutput("enable_radio() completed");
                }
                removeReceiver(receiver);
                return;
            }
            sleep(POLL_RADIO_TIME);
        }
        int firedFlags=receiver.getFiredFlags();
        removeReceiver(receiver);
        fail(String.format("enable_radio() timeout . "));
    }
     /**
     * Disables radio and make sure that the radio is off.
     *
     * @param adapter The BT adapter.
     */
    public void disable_radio(BluetoothAdapter adapter) {
        int mask = (BluetoothReceiver.STATE_RADIO_OFF_FLAG);
        long start = -1;
        BluetoothReceiver receiver = getBluetoothReceiver(mask);
         if(adapter.isRadioEnabled())
        {
            assertTrue(adapter.isRadioEnabled());
            start= System.currentTimeMillis();
            assertTrue(adapter.disableRadio());
        }
        else if (!adapter.isRadioEnabled()){
            assertFalse(adapter.isRadioEnabled());
            removeReceiver(receiver);
        ...
        while(System.currentTimeMillis() - s < ENABLE_DISABLE_TIMEOUT) {
            if(!adapter.isRadioEnabled() && (receiver.getFiredFlags() & mask) == mask){
                ...
                long finish=System.currentTimeMillis();
                if(start !=-1 && finish!=-1){
                 writeOutput(String.format("disable_radio() completed in %d ms", (finish - start)));
                } else {
                    writeOutput("disable_radio() completed");
                }
                removeReceiver(receiver);
                return;
            }
       ...
       int firedFlags=receiver.getFiredFlags();
        removeReceiver(receiver);
        fail(String.format("disable_radio() timeout"));
    }
    ...
    private BluetoothReceiver getBluetoothReceiver(int expectedFlags) {
        String[] actions = {
                ...
                BluetoothAdapter.ACTION_RADIO_STATE_CHANGED};
