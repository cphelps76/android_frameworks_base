...
public interface IMountService extends IInterface {
    ...
    public static abstract class Stub extends Binder implements IMountService {
        ...
        static final int TRANSACTION_getVolumeLabel = IBinder.FIRST_CALL_TRANSACTION + 34;

        static final int TRANSACTION_getVolumeFSLabel = IBinder.FIRST_CALL_TRANSACTION + 35;
        
        static final int TRANSACTION_isUsbMassStorageAllEnabled = IBinder.FIRST_CALL_TRANSACTION + 36;    

	 static final int TRANSACTION_getAllVolume = IBinder.FIRST_CALL_TRANSACTION + 37;

	 static final int TRANSACTION_getVolumedevtype = IBinder.FIRST_CALL_TRANSACTION + 38;

	 static final int TRANSACTION_getVolumedevnode = IBinder.FIRST_CALL_TRANSACTION + 39;

	 static final int TRANSACTION_getVolumeUUID = IBinder.FIRST_CALL_TRANSACTION + 40;
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply,
            ...
            switch (code) {
                ...
                case TRANSACTION_getVolumeFSLabel: {
                    data.enforceInterface(DESCRIPTOR);
                    String mountPoint;
                    mountPoint = data.readString();
                    String state = getVolumeFSLabel(mountPoint);
                    reply.writeNoException();
                    reply.writeString(state);
                    return true;
                 }
		   case TRANSACTION_getAllVolume: {
                    data.enforceInterface(DESCRIPTOR);
                    java.util.ArrayList<java.lang.String>  state = getAllVolume();
                    reply.writeNoException();
                    reply.writeStringList(state);
                    return true;
		   }
		   case TRANSACTION_getVolumedevtype: {
                    data.enforceInterface(DESCRIPTOR);
                    String mountPoint;
                    mountPoint = data.readString();
                    int resultCode = getVolumedevtype(mountPoint);
                    reply.writeNoException();
                    reply.writeInt(resultCode);
                    return true;
		   }
		   case TRANSACTION_getVolumedevnode: {
                    data.enforceInterface(DESCRIPTOR);
                    String mountPoint;
                    mountPoint = data.readString();
                    int resultCode = getVolumedevnode(mountPoint);
                    reply.writeNoException();
                    reply.writeInt(resultCode);
                    return true;
		   }
		   case TRANSACTION_getVolumeUUID: {
                    ...
                    String mountPoint;
                    mountPoint = data.readString();
                    String state = getVolumeUUID(mountPoint);
                    reply.writeNoException();
                    reply.writeString(state);
                    return true;			
                }
