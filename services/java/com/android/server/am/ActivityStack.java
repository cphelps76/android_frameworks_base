...
final class ActivityStack {
    ...
    static boolean mobileApkSpecialProcess = false;
    static String PutClassName;
    ...
    */
    ...
    native int enableFreeScaleMBX();
    native int disableFreeScaleMBX();
        ...
        if (next.app != null && next.app.thread != null) {
            ...
            try {
                ...
                if(next.realActivity != null) {
                    ...
                    PutClassName = resumeClassName;
                    Log.d(TAG, "resumeClassName is " + resumeClassName);
                    Log.d(TAG, "resumePackageName is " + resumePackageName);
                    if (prev != null){
                        String prevClassName = prev.realActivity.getClassName();
                        Log.d(TAG, "prevClassName is " + prevClassName);
                    } else {
                        Log.d(TAG, "prevClassName is null");
                    }
                        ...
                        Runnable disrunnable = new Runnable(){
                            ...
                            public void run(){   
                                synchronized(FreescaleLocked){
                                    Log.d(TAG, "--------------------disableFreeScaleMBX runnable");
                                    disableFreeScaleMBX();
                                    Settings.Secure.putString(resolver, "class_name", PutClassName);
                                }
                            }
                        };
                           ...
                           resumeClassName , resumePackageName)){
                            ...
                            if(readXml(mContext , com.android.internal.R.xml.blackpackagefilter , 
                               ...
                               Thread DisableFreeScaleThread = new Thread(disrunnable); 
                                ...
                                try{
                                    if(Settings.Secure.getInt(resolver, resumeClassName) == 1){
                                
                                        Log.i(TAG, "Setting found the classname in secure table!");
                                    ...
                                    Settings.Secure.putString(resolver, "class_name", resumeClassName);
                                    Log.i(TAG, "Setting not found the classname in secure table!");
                                    Runnable runnable = new Runnable(){
                                        public void run(){   
                                            synchronized(FreescaleLocked){
                                                Log.d(TAG, "--------------------enableFreeScaleMBX runnable");  
                                                enableFreeScaleMBX();
                                                Settings.Secure.putString(resolver, "class_name", PutClassName);
                                           }
                                       }
                            ...
                            Log.w(TAG , "whitepackagefilter has find classname or packagename in resumeactivity");
                        }
                    }
               }
            }
            ...
            if(SystemProperties.getBoolean("media.p2pplay.enable", false)){
                ...
                if(!readXml(mContext , com.android.internal.R.xml.whitepackagefilter , 
                   ...
                   if(readXml(mContext , com.android.internal.R.xml.blackpackagefilter ,
                        ...
                        try {
                            if(Settings.Secure.getInt(resolver, className) == 1){
                                Thread DisableFreeScaleThread = new Thread(runnable); 
                                DisableFreeScaleThread.start();
                            }
                        }
                        ...
                        Log.w(TAG , "blackpackagefilter  not found packagename or classname");
                    }
                }
                else{
                    Log.w(TAG , "whitepackagefilter  has packagename or classname");	
                }
            }
        }
            ...
            if(setClassName) {
                try {
                    Runnable runnable = new Runnable() {
                        public void run() {
                            synchronized(FreescaleLocked){
                                Log.d(TAG, "--------------------enableFreeScaleMBX runable");
                                enableFreeScaleMBX();
                                Settings.Secure.putString(resolver, "class_name", PutClassName);
                            }  
                        }
                ...
                catch(NullPointerException err_null) {
                    Log.i(TAG, "Settings.Secure.putString get NullPointerException ERROR!!!");
                }
            }
            else
    ...
    final boolean readXml(Context mContext , int file , String classname , String packagename){
        ...
        if(map.size() > 0){
            ...
            Log.w(TAG , "map not have packagename");
        return backValue;
    }
    ...
    final int startActivityMayWait(IApplicationThread caller, int callingUid,
        ...
        //1024x768 resolution screen need resize surfaceview in order to support special apks
        if ( ((1024 == p.x) && (768 == p.y)) || ((1024 == p.y) && (768 == p.x)) ) {
            //dou di zhu apk
            if (aInfo != null && aInfo.applicationInfo != null
                    && "com.npc.landlord3".equals(aInfo.applicationInfo.packageName)) {
                SystemProperties.set("service.apk.resize", "true");
                //Slog.v(TAG, "start activity package name: " + aInfo.applicationInfo.packageName);
            }
            else {
                if (SystemProperties.getBoolean("service.apk.resize", false))
                    SystemProperties.set("service.apk.resize", "false");
            }
        }
