...
class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    //private final Runnable mBTDelayeddisableradio = new Runnable() {
    //    public void run() {
    //        disableRadio();
    //    }
    //};
            ...
            if (BluetoothAdapter.ACTION_LOCAL_NAME_CHANGED.equals(action)) {
            	...
            	if(!("true".equals(SystemProperties.get("bt.keep_on.enable")))) {
	                ...
	                if (isEnabled() || mState == BluetoothAdapter.STATE_TURNING_ON || isRadioEnabled()) {
	                    mHandler.sendMessage(mHandler.obtainMessage(MESSAGE_GET_RX_COUNT,
	                           0, 0));
	                    mDelayedHandler.postDelayed(mBTtimer, 10 * 1000);
	                } //else if (isRadioEnabled()){
	                    //mDelayedHandler.postDelayed(mBTDelayeddisableradio, 10 * 1000);
	                //}
            	}
                ...
                synchronized(mReceiver) {
                    if (mEnableExternal && isBluetoothPersistedStateOnBluetooth()) {
                        ...
                        if (DBG) Log.d(TAG, "enable Bluetooth radio for GPS");
			            enableRadio();
                    }
                }
